defmodule ETitleWeb.AccountAuth do
  @moduledoc """
  Handles account authentication and authorization.
  """
  ...
  def log_out_account(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      ETitleWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      ETitleWeb.Endpoint.broadcast(account_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule ETitleWeb.PageLive do
        use ETitleWeb, :live_view

        on_mount {ETitleWeb.AccountAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{ETitleWeb.AccountAuth, :require_authenticated}] do
  ...
  def on_mount(:require_authenticated_staff, _params, session, socket) do
    socket = mount_current_scope(socket, session)

    if socket.assigns.current_scope && socket.assigns.current_scope.account.type == :staff do
      {:cont, socket}
    else
      socket =
        socket
  ...
  def require_authenticated_account_staff(conn, _opts) do
    if conn.assigns.current_scope && conn.assigns.current_scope.account.type == :staff do
      conn
    else
      conn
