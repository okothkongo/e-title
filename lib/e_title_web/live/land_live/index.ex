defmodule ETitleWeb.LandLive.Index do
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      ...
      >
        ...
        <:col :let={{_id, land}} label="Gps coordinates">{land.gps_coordinates}</:col>
        <:col :let={{_id, land}} label="Status">{land.status}</:col>
        <:col :let={{_id, land}} label="Blockchain hash">{land.blockchain_hash}</:col>
        <:action :let={{_id, land}}>
          <div class="sr-only">
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok,
     ...
     |> stream(:lands, Lands.list_lands(socket.assigns.current_scope))}
  end
  ...
  def handle_info({type, %ETitle.Lands.Land{}}, socket)
    ...
    {:noreply,
     stream(socket, :lands, Lands.list_lands(socket.assigns.current_scope), reset: true)}
  end
end
