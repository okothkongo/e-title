defmodule ETitleWeb.LandLive.Form do
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      ...
      <.form for={@form} id="land-form" phx-change="validate" phx-submit="save">
        ...
        <.input field={@form[:gps_coordinates]} type="text" label="Gps coordinates" />
        <.input
          field={@form[:status]}
          type="select"
          label="Status"
          prompt="Choose a value"
          options={Ecto.Enum.values(ETitle.Lands.Land, :status)}
        />
        <.input field={@form[:blockchain_hash]} type="text" label="Blockchain hash" />
        <footer>
  ...
  defp apply_action(socket, :new, _params) do
    land = %Land{owner_id: socket.assigns.current_scope.user.id}
