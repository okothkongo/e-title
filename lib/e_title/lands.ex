defmodule ETitle.Lands do
  ...
  def subscribe_lands(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(ETitle.PubSub, "user:#{key}:lands")
  end

  defp broadcast(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(ETitle.PubSub, "user:#{key}:lands", message)
  end
  ...
  def list_lands(%Scope{} = scope) do
    Repo.all_by(Land, owner_id: scope.user.id)
  end
  ...
  def get_land!(%Scope{} = scope, id) do
    Repo.get_by!(Land, id: id, owner_id: scope.user.id)
  end
  ...
  def update_land(%Scope{} = scope, %Land{} = land, attrs) do
    true = land.owner_id == scope.user.id

    with {:ok, land = %Land{}} <-
           land
           |> Land.changeset(attrs, scope)
           |> Repo.update() do
      broadcast(scope, {:updated, land})
      {:ok, land}
    end
  end
